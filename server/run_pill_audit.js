...
async function runAudit() {
  ...
  try {
    ...
    // Since Supabase doesn't support complex aggregation via REST, fetch all and check locally
    const { data: allPlayData, error: fetchErr } = await supabase
      .from('pill_plays')
      .select('pill_id, player_id, id, won, created_at, locked_at');

    if (fetchErr) {
      console.log('Error fetching plays:', fetchErr.message);
      return;
    }
    ...
    if (dups.length > 0) {
      ...
      dups.forEach(dup => {
        console.log(`\n  Pill: ${dup.pill_id}`);
        console.log(`  Player: ${dup.player_id}`);
        console.log(`  Number of entries: ${dup.count}`);
        dup.duplicate_records.forEach((rec, i) => {
          console.log(`    [${i+1}] ID: ${rec.id}, Won: ${rec.won}, Locked: ${rec.locked_at}`);
        });
      });
    } else {
